...
export const Dispositivi = () => {
	...
	const getTerminals = async () => {
		await utils.checkTokenValidity(); /* Check if token is valid and get a new one if it's expired or not present and put it in the sessionStorage, call the API to get the terminals list*/
	...
	const deleteTerminal = async () => {
		await utils.checkTokenValidity(); /* Check token validity, get a new one if expired or not present, call the API to delete a terminal*/
	...
	useEffect(() => {
		let r: any =
			[]; /* If terminals change, (aka I just called the API) create an array with the data I want to put in the table and setRows to that array */
	...
	useEffect(() => {
		if (process.env.REACT_APP_IS_USING_MOCK === 'true') {
			/* If token has changed call the API again (backup) if something else fails*/
			setTerminals({
				subscribers: [
					{
						acquirerId: '4585625',
